...
const CreateMovieDialog = ({ onClose }: DialogProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitError(null);
    try {
      const id = await CreateMovie({ name: name, summary: summary }, apiKey);
      setName("");
      setSummary("");
      setApiKey("");
      redirect(`/movie/${id}`);
    } catch (err) {
      setSubmitError("Failed to create movie.");
    } finally {
      setSubmitting(false);
    }
  };
